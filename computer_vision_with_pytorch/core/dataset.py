...
def dataset(dataset_type: str, dataset_name: str):
    '''
    This function downloads the required dataset at initial execution, then it transforms the data into Tensors and
    normalizes them and returns the desired dataset
    For example, if you wanted a training dataset for a model using images from CIFAR10, you would simply have to type
    dataset("train", "cifar10")

    :param dataset_type: ("train" / "test" / "validation")
    :param dataset_name: ("mnist" / "cifar10" / "cifar100")
    :return: torchvision.datasets.SOMEDATASET(root='./data', train=False, download=False, transform=transf)
    '''
